...
function editUsuario2($nome, $cpf, $id, $ativo) {
    $conn = conectar();
    try {
        $conn->beginTransaction();
        $sql = "UPDATE usuario SET nomeUsuario = :nome, cpf = :cpf, ativo = :ativo WHERE idusuario = :id";
...
function listarGeralJoin($campos, $tabela, $joins = '')
{
    $conn = conectar();
    try {
        $sqlLista = $conn->prepare("SELECT $campos FROM $tabela $joins");
        $sqlLista->execute();
